...
from catalogueDonnées.models import Jeu_De_Donnee, Ressource, Mot_Cle, Organisation, Group, LogMoissonage
...
def stockageJeuDeDonnée(config):

    logMoisson = LogMoissonage.objects.get_or_create(
                configuration = config,
                commence = timezone.now(),
                statut = 'running'
        )[0]
    ...
    if jeuDeDonnées:     
        ...
        for jeuDeDonnée in jeuDeDonnées:
            ...
            # get_or_create retourne l'instance et un flag.
            # [0] pour obtenir uniquement l'instance.
            )[0]

            j = Jeu_De_Donnee.objects.get_or_create(
                organisation=org,
            ...
            if ressources_objs:
                Ressource.objects.bulk_create(ressources_objs, batch_size=100, ignore_conflicts=True)
            ...
            if mots_objs:
                Mot_Cle.objects.bulk_create(mots_objs, batch_size=100, ignore_conflicts=True)
            ...
            if groups_objs:
                Group.objects.bulk_create(groups_objs, batch_size=100, ignore_conflicts=True)
        ...
        logMoisson.complete = timezone.now()
        logMoisson.records_harvested = len(jeuDeDonnées)
        logMoisson.error_message = ""
        logMoisson.statut = 'success'
        logMoisson.save()

    else:
        logMoisson.complete = timezone.now()
        logMoisson.records_harvested = 0
        logMoisson.error_message = ""
        logMoisson.statut = 'error'
        logMoisson.save()  
